...
export async function getQuote(symbol: string): Promise<QuoteData> {
    try {
        ...
        const data = await fetchJSON<{ c?: number; dp?: number }>(url, 15);
        return { c: data?.c, dp: data?.dp };
    } catch (e) {
        console.error("getQuote error", symbol, e);
        return {};
    }
}

// Fetch profile metrics for a single symbol (market cap, PE)
export async function getKeyMetrics(
    symbol: string
): Promise<{ marketCap?: string; peRatio?: string }> {
    try {
        const token =
            process.env.FINNHUB_API_KEY ?? NEXT_PUBLIC_FINNHUB_API_KEY;
        if (!token) return {};
        ...
        const mcRaw: number | undefined = profile?.marketCapitalization;
        const peRaw: number | undefined =
            financials?.metric?.["peNormalizedAnnual"] ??
            financials?.metric?.["peBasicExclExtraTTM"];

        const marketCap =
            typeof mcRaw === "number" && isFinite(mcRaw)
                ? `$${mcRaw.toLocaleString("en-US", {
                      maximumFractionDigits: 0,
                  })}B`
                : undefined;
        const peRatio =
            typeof peRaw === "number" && isFinite(peRaw)
                ? peRaw.toFixed(1)
                : undefined;

        return { marketCap, peRatio };
    } catch (e) {
        console.error("getKeyMetrics error", symbol, e);
        return {};
    }
}

// Batch fetch helper for multiple symbols
export async function getBatchQuotesAndMetrics(
    ...
    await Promise.all(
        clean.map(async (sym) => {
            ...
            const currentPrice =
                typeof quote?.c === "number" ? quote.c : undefined;
            const changePercent =
                typeof quote?.dp === "number" ? quote.dp : undefined;
